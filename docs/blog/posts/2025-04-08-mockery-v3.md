...
[Mockery v3](https://vektra.github.io/mockery/latest-v3/) will make obsolete all other interface-based code generation frameworks that currently exist in the Go community. A bold statement you might say? Fortunately, I have quite the justification. 
...
![](https://assets.topofmind.dev/images/blog/2025-04-08-mockery-v3/github-checkmarks.png)
...
That number 5 is a real drag because almost every package is going to have _some_ kind of external dependency, and to generate implementations of interfaces, you usually need to include those external types in your method signatures. So if we call `//go:generate` over and over and over again, we keep _re-parsing_ the exact same dependencies every single time.

So I had a novel thought: what if we _cached_ dependency parsing?
...
!!! tip "10x Performance Increase"
    ...
    Other testimonials from users of other code generation projects found around a whole [gaht dang _order of magnitude_ ](https://github.com/vektra/mockery/issues/936#issuecomment-2694167799)(~10) increases in performance after switching to mockery.
...
I feel like Steve Ballmer when he got up on stage chanting "developers :clap: developers :clap: developers :clap:." But for me it's all about templates.

![](https://assets.topofmind.dev/images/blog/2025-04-08-mockery-v3/steve-ballmer.gif)

Mockery's codebase in v1 was, in a manner of speaking, a rat's nest that had grown organically. I actually inherited this project from another talented fellow, but after some historic cruft and missteps of my own, I created somewhat of a monster. Mockery _did_ use templates, but they were scattered around in various pieces of the codebase and generally impossible to follow. v2 improved upon the ergonomics and performance, but it still suffered from this historical cruft that I had to support.

My first task in v3 was to unify these disparate templates into a single `mockery.tmpl` file. Once I did that, I thought to myself that it might be a cool idea if I supported `https://github.com/matryer/moq` style of mocks into mockery's templating system. I tried it out and after some tinkering, it kind of just worked.
...
It then further begged the question: why does _ANY_ code generation project exist anymore? We can port all of them to the mockery framework. I've solved the configuration and parsing problem and unlocked a faster processing speed than any other code generation project out there.  `gowrap` is the closest project with its ability for its users to specify remote templates, but it still has performance problems.

## The Case For Mockery

I hope by this point I've convinced you that mockery is the Go community's future for interface-based code generation. I've engineered v3 to empower template developers to maintain and distribute their own templates, rendered with mockery, out into the world. My intention is to eventually subsume all other code generation projects out there into the mockery framework not through force, but through sheer friction reduction. Mockery should be the obvious choice.

Anyway, give [Mockery v3](https://vektra.github.io/mockery/latest-v3/) a try and let me know your thoughts!

![](https://assets.topofmind.dev/images/blog/2025-04-08-mockery-v3/steve-ballmer-dancing-lol.gif)
